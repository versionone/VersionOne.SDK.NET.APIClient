...
namespace VersionOne.SDK.APIClient.Meta
{
    public class MetaModel : IMetaModel
    {
        private readonly ICanSetApi _connector;
        private readonly IDictionary _map = new Hashtable();
        private Version _version;
        private string _versionString = null;

        public MetaModel(ICanSetApi connector)
            : this(connector, false)
        {
        }

        public MetaModel(ICanSetApi connector, bool hookup)
        {
        ...
        {
            type = null;
            try { type = LookupAssetType(token); }
            catch (Exception) { }
            return type != null;
        }
        ...
        {
            def = null;
            try { def = LookupAttributeDefinition(token); }
            catch (Exception) { }
            return def != null;
        }
        ...
        private IAssetType HookupAssetType(string token)
        {
            ...
            using (Stream stream = _connector.UseMetaApi().GetData(token))
                doc.Load(stream);
        ...
        {
            XmlDocument doc = new XmlDocument();
            using (Stream stream = _connector.UseMetaApi().GetData(assettypetoken + "/" + name))
                doc.Load(stream);
        ...
        {
            XmlDocument doc = new XmlDocument();
            using (Stream stream = _connector.UseMetaApi().GetData())
                doc.Load(stream);
